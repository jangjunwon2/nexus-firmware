...
void CommManager::handleEspNowRecv(const esp_now_recv_info_t* recv_info, const uint8_t* incomingData, int len) {
    ...
    // _myDeviceId를 콜백 시작 시 로컬 복사 → ID 변경과의 경쟁 조건 차단
    const uint8_t myId = _myDeviceId;
    ...
    // [NEW] 취소 패킷 수신 감지 (송신기 B 홀드 → 즉시 시퀀스 중단)
    const Comm::CancelPacket* cancelPkt = nullptr;
    if (Comm::verifyCancelPacket(incomingData, len, cancelPkt)) {
        Log::Info(PSTR("COMM: CANCEL received — targetId=%d, myId=%d"), cancelPkt->targetId, myId);
        if (cancelPkt->targetId == 0 || cancelPkt->targetId == myId) {
    ...
    if (Comm::verifyHoldPacket(incomingData, len, holdPkt)) {
        Log::Info(PSTR("COMM: HOLD verify OK -> targetId=%d myId=%d active=%d pwm=%d"),
                  holdPkt->targetId, myId, holdPkt->holdActive, holdPkt->pwmValue);
        if (holdPkt->targetId == myId || holdPkt->targetId == 0) {
            if (_modeManager) _modeManager->handleHold(holdPkt->pwmValue, holdPkt->holdActive);
        } else {
            Log::Warn(PSTR("COMM: HOLD ignored — targetId=%d != myId=%d"), holdPkt->targetId, myId);
        }
        return;
    }
    ...
    if (!Comm::verifyCommPacket(incomingData, len, pkt, myId, forMe)) return;
    if (!forMe) return;
