...
void ModeManager::handleEspNowCommand(const uint8_t* senderMac, const Comm::CommPacket& pkt, uint32_t rxTime) {
    ...
    if (_currentMode == DeviceMode::MODE_ID_SET) {
        Log::Info(PSTR("MODE: Ignored FIRE command because device is in ID_SET mode."));
        return;
    }

    if (pkt.packetType == Comm::FIRE_COMMAND) {
        // [COMM TEST] 무발사 핑(스텝 1개, delay/play/pwm 전부 0)은 저장/실행 없이 ACK만 응답.
        // → 송신기의 사전 통신 점검이 'replay last' 데이터를 덮어쓰거나 출력을 건드리지 않음.
        bool isCommTestPing = (pkt.stepCount == 1 &&
                               pkt.steps[0].delayMinutes == 0 && pkt.steps[0].delaySeconds == 0 &&
                               pkt.steps[0].playSeconds == 0 && pkt.steps[0].pwmValue == 0);
        if (isCommTestPing) {
            _commManager->sendAck(senderMac, pkt, rxTime);
            return;
        }

        // [지연 ACK] 송신기는 FIRE 버스트(~6ms) 송신 중엔 반이중이라 수신 불가 → 즉시 ACK하면 유실됨.
        // 버스트가 끝날 시간을 두고 메인루프(update)에서 ~12ms 뒤 ACK 송출. 같은 txMicros는 1회만.
        ...
        if (isNewCommandSequence) {
            ...
            Log::Info(PSTR("MODE: FIRE received. ID: %u, steps: %d"), pkt.txButtonPressMicros, pkt.stepCount);
            if (_isPlaySequenceActive) stopPlaySequence();
            _currentCommandId = pkt.txButtonPressMicros;
            // [핵심 FIX] saveLastSequence(NVS 플래시 쓰기) 제거.
            // ESP32 플래시 쓰기는 양쪽 코어를 멈춰(캐시 비활성) WiFi 송신을 깨뜨림 → 첫 ACK 유실
            // (성공 시 항상 "try 2"가 뜨던 원인). 게다가 loadLastSequence는 어디서도 호출 안 되는
            // 죽은 기능이라 저장 자체가 불필요. 발사는 즉시 실행.
            startPlaySequence(pkt.steps, pkt.stepCount, elapsedSinceButtonUs);
        }
        // 중복(버스트/재시도)은 조용히 무시 — ACK는 위에서 이미 처리됨
    }
}
...
void ModeManager::startPlaySequence(const ExecutionStep* steps, uint8_t stepCount, uint32_t elapsedSinceButtonUs) {
    ...
    // 버튼 누름~패킷 송출 경과 시간만큼 첫 번째 딜레이에서 차감
    // uint64_t로 계산: delayMinutes > 71이면 uint32_t가 오버플로되어 즉시 발사 버그 발생
...
void ModeManager::updatePlaySequence() {
    ...
    unsigned long currentTime = millis();
    // 롤오버 안전 비교: (currentTime - _phaseStartMs) 부호없는 뺄셈은 올바르게 처리됨
...
void ModeManager::update() {
    ...
    // [홀드] 누르는 동안 지속 출력 — 콜백이 세운 플래그를 메인루프에서 안전하게 출력 반영
    serviceHoldOutput();
    ...
    if (manualPending) {
        if (_currentMode == DeviceMode::MODE_TEST || _currentMode == DeviceMode::MODE_WIFI) {
            ...
            if (_webManager) _webManager->broadcastTestStart(manualDelay, manualPlay);
        ...
        case DeviceMode::MODE_PAIRING:
            {
                ...
                uint8_t masterMac[6];
                bool hasMac = Utils::loadMasterMac(masterMac);
                Log::Info(PSTR("MODE: Entered Pairing Mode. Press 'PAIRING' on Transmitter... (HW Ch: %d, Has Master MAC: %d)"),
                          primaryChan, hasMac);
                if (hasMac) {
                    Log::Info(PSTR("  Current Registered Master: %02X:%02X:%02X:%02X:%02X:%02X"),
...
void ModeManager::handleChannelCommit(uint8_t channel) {
    if (channel != 1 && channel != 6 && channel != 11) {
        Log::Warn(PSTR("MODE: Ignoring invalid channel commit: %d"), channel);
        return;
    }
...
// [NEW] 홀드 패킷 수신 (ESP-NOW 콜백) → 플래그만 세팅. 실제 출력은 update()의 serviceHoldOutput().
// active=1: 유지(킵얼라이브), active=0: 명시적 OFF(뗄 때) → 안전차단도 해제.
...
// [NEW] 홀드 출력 서비스(메인루프). 킵얼라이브 끊기면 자동 OFF, 안전 최대시간 초과 시 차단.
void ModeManager::serviceHoldOutput() {
    const unsigned long HOLD_TIMEOUT_MS = 250;   // 마지막 수신 후 끊김 판정
    const unsigned long HOLD_MAX_MS     = 30000; // 안전 최대 연속 ON

    // 콜백↔메인루프 공유 상태를 스냅샷으로 복사 (spinlock)
    taskENTER_CRITICAL(&_holdMux);
    bool holdReq             = _holdReq;
    uint8_t holdPwm          = _holdPwm;
    unsigned long lastHoldMs = _lastHoldMs;
    taskEXIT_CRITICAL(&_holdMux);

    // 신호 끊김 → OFF + 차단 해제
    if (holdReq && (millis() - lastHoldMs > HOLD_TIMEOUT_MS)) {
        taskENTER_CRITICAL(&_holdMux);
    ...
    if (holdReq && !_holdOn) {
        _holdOn = true;
        _holdStartMs = millis();
        if (_isPlaySequenceActive) stopPlaySequence(); // 타이머 재생과 충돌 방지
        if (_hwManager) {
            _hwManager->setMosfets(holdPwm);
            _hwManager->setLedPattern(LedPatternType::LED_ON);
        }
        Log::Info(PSTR("MODE: HOLD ON (pwm %d)"), holdPwm);
    }
    // 안전 최대시간 초과 → 강제 OFF + 차단(해제 전까지 재ON 금지)
    if (_holdOn && (millis() - _holdStartMs > HOLD_MAX_MS)) {
        taskENTER_CRITICAL(&_holdMux);
        _holdReq = false; _holdBlocked = true;
        taskEXIT_CRITICAL(&_holdMux);
        holdReq = false;
...
void ModeManager::updateModeRfScan() {
    ...
    if (_scanStepIdx < 3) {
        ...
        if (_scanReportAttempts == 0) {
            _rfScanBestCh = 1; // 매 스캔 세션 시작 시 초기화 (static 잔여값 오염 방지)
            for (int i = 0; i < 3; i++) {
...
const char* ModeManager::getModeName(DeviceMode mode) const {
    switch (mode) {
        ...
        case DeviceMode::MODE_RF_SCAN: return "RF_SCAN";
