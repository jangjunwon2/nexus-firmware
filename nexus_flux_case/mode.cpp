/**
 * @file mode.cpp
 * @brief ModeManager Class Implementation
 * @version 9.5.0
 * @date 2024-08-30 (Updated with Pairing Mode)
 */
#include "mode.h"
#include "hardware.h"
#include "comm.h"
...
ModeManager::ModeManager(HardwareManager* hwManager, CommManager* commManager, WebManager* webManager)
    : _hwManager(hwManager), _commManager(commManager), _webManager(webManager),
      ...
      _currentCommandId(0),
      _planStepCount(0), _currentStepIndex(0),
      _idSetState(IdSetState::IDLE),
      _temporaryId(0),
      _idSetLastInputTime(0),
      _isPlaySequenceActive(false), _isManualOperationActive(false),
      _isDelayPhase(false), _phaseEndTime(0),
      ...
      _pairingStartTime(0) // [NEW] 페어링 타임아웃용 타이머 초기화
{
...
void ModeManager::handleEspNowCommand(const uint8_t* senderMac, const Comm::CommPacket& pkt, uint32_t rxTime) {
    ...
    // MachineType 필터링: ALL이거나 내 타입(MAGNET)과 같아야 함
...
void ModeManager::triggerManualRun(uint32_t delayMs, uint32_t playMs) {
    if (_currentMode == DeviceMode::MODE_TEST || _currentMode == DeviceMode::MODE_WIFI) {
        ...
        _currentCommandId = 0; // Manual run
        ...
        manualStep.pwmValue = 100; // 기본값 100
...
void ModeManager::update() {
    ...
    switch (_currentMode) {
        ...
        case DeviceMode::MODE_PAIRING:  updateModePairing(); break; // [NEW] 페어링 대기열 처리
        default: break;
    }
}
...
void ModeManager::handleButtonEvent(ButtonEventType event) {
    ...
    if (_currentMode == DeviceMode::MODE_ID_SET) {
        ...
        // [NEW] ID 설정 모드 대기 상태에서 EXEC 버튼을 누르면 페어링 모드로 진입!
        else if (event == ButtonEventType::EXEC_BUTTON_PRESS) switchToMode(DeviceMode::MODE_PAIRING);
        return;
    }

    // [NEW] 페어링 모드 중에 아무 버튼이나 누르면 취소
        ...
        case ButtonEventType::EXEC_BUTTON_PRESS:
            if (_currentMode == DeviceMode::MODE_NORMAL) {
                if (_isPlaySequenceActive) {
                    ...
                    ExecutionStep savedSteps[MAX_EXECUTION_STEPS];
                    uint8_t savedCount = 0;
                    if (Utils::loadLastSequence(savedSteps, savedCount)) {
                        Log::Info(PSTR("MODE: Button pressed. Replaying last sequence (%d steps)."), savedCount);
                        startPlaySequence(savedSteps, savedCount, 0, 0);
                    } else {
                        Log::Info(PSTR("MODE: No saved sequence. executing default 10s run."));
                        savedCount = 1;
                        savedSteps[0].delayMinutes = 0;
                        savedSteps[0].delaySeconds = 0;
                        savedSteps[0].playSeconds = 10;
                        savedSteps[0].pwmValue = 100;
                        startPlaySequence(savedSteps, savedCount, 0, 0);
                    }
                }
            }
...
void ModeManager::enterModeLogic(DeviceMode mode) {
    switch (mode) {
        ...
        case DeviceMode::MODE_PAIRING:
            ...
            if (_commManager) _commManager->setPairingMode(true); // 통신매니저에게 가로채기 허가
...
void ModeManager::notifyPairingSuccess() {
    ...
    if (_hwManager) _hwManager->setLedPattern(LedPatternType::LED_BOOT_SUCCESS); // 성공의 의미로 1초간 켜짐
    switchToMode(DeviceMode::MODE_NORMAL);
}
