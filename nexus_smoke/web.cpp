...
pg_home:["Nexus Smoke","Nexus 스모크","Nexus 烟雾","Nexus スモーク","Nexus Rauch","Nexus Humo","Nexus Fumée"],
...
void WebManager::handleRoot(AsyncWebServerRequest* request) {
    ...
    String html = getPageHeader("Nexus Smoke", "pg_home");
...
void WebManager::handleManualPage(AsyncWebServerRequest* request) {
    ...
    <details open>
      ...
      <div class='mb'>
        ...
        <table class='st'>
          ...
          <tr><td>켜짐 (지속)</td><td>출력 중 — 스모크 모듈(MOSFET)이 작동하여 연기가 나오고 있습니다</td></tr>
    ...
    <details>
      ...
      <div class='mb'>
        <p>리모컨 없이 기기 본체의 EXEC 버튼으로 직접 연기를 발생시킬 수 있습니다.</p>
        <p style='margin-top:10px;margin-bottom:4px;'><b>대기 중 (정상 상태):</b></p>
        <p style='margin:0 0 0 8px;'>누르는 동안 100% 출력 (가열 작동). 손을 떼면 즉시 정지</p>
        <p style='margin-top:8px;margin-bottom:4px;'><b>리모컨 시퀀스 실행 중:</b></p>
        <p style='margin:0 0 0 8px;'>누르면 현재 실행 중인 시퀀스를 강제 중단합니다</p>
        <div class='wb' style='margin-top:10px;'>⚠ 코일 과열 및 연속 조작 방지를 위해 EXEC 버튼은 500ms 쿨다운이 있습니다.</div>
      </div>
    </details>
    <details>
      ...
      <div class='mb'>
        ...
        <table class='st'>
          ...
          <tr><td><span class='kb'>▶</span> (플레이) 짧게 누름</td><td>리모컨만 홈 화면으로 복귀. <b>수신기 출력(가열)은 계속 진행</b>됩니다.</td></tr>
          <tr><td><span class='kb'>B</span> 0.5초 이상 누름</td><td>모든 수신기에 즉시 취소 신호 전송 → 가열 작동 즉시 중단.</td></tr>
        </table>
    ...
    <details>
      ...
      <div class='mb'>
        ...
        <ol class='sl'>
          ...
          <li>스마트폰 Wi-Fi에서 <b>Nexus_Smoke</b>에 연결</li>
    ...
    <details>
      ...
      <div class='mb'>
        ...
        <table class='st'>
          ...
          <tr><td>실행 시간 (초)</td><td>테스트 시 출력(가열) 지속 시간</td></tr>
    ...
    <details>
      ...
      <div class='mb'>
        <p><b>Nexus Smoke</b> 수신기는 안개 발생용 소형 발열 코일 구동에 적합한 <b>디지털 ON/OFF 출력</b> 방식을 사용합니다.</p>
        <p style='margin-bottom:4px;'><b>대표적인 마술 연출 및 응용 예시:</b></p>
        <ol class='sl'>
          <li><b>손안의 연기 (Smoke from Hands)</b>: 소매나 손목에 소형 연기 기믹을 장착하여, 마술사가 아무것도 없는 손에서 연기를 뿜어내는 듯한 신비로운 효과를 낼 수 있습니다.</li>
          <li><b>기믹 박스 / 컵 연기 (Smoke in Box/Cup)</b>: 컵이나 상자 내부에 기믹을 고정하고 리모컨을 통해 작동시켜, 컵을 들어 올렸을 때 연기가 가득 찬 연출을 구현합니다.</li>
          <li><b>카드/동전 연출 (Card/Coin Magic)</b>: 동전이나 카드가 사라지는 순간 연기가 뿜어 나와 사라짐을 시각적으로 강조합니다.</li>
        </ol>
        <div class='tb'>💡 <b>디지털 출력 및 송신기 설정</b>: 전압을 온/오프 상태로만 제어하므로 송신기(리모컨)에서 <b>POWER(출력 세기)를 100%</b>로 설정하고 사용하시는 것이 올바른 동작을 보장합니다.</div>
        <div class='wb'>⚠ <b>코일 파손 주의</b>: 스모크 오일(연기액)이 없는 상태에서 공가열하거나 재생 시간(PLAY)을 5초 이상으로 길게 유지하면 발열 코일이 타버려 단선될 위험이 큽니다. 가급적 재생 시간은 <b>1.5초~3초</b> 이내로 권장하며, 가동 전 항상 오일 잔량을 확인하세요.</div>
      </div>
    </details>
    </div>
    )rawliteral";
                    ...
                    switch (data.status) {
                        ...
                        case "failed":
                            if (data.reason === 15 || data.reason === 2 || data.reason === 8) {
                                ...
                                showMessage(wt('msg_conn_fail_env'), 'error');
                            }
                            break;
                        case "disconnected":
            ...
            function handleScanResult(data) {
                ...
                if (data.networks && data.networks.length > 0) {
                    data.networks.slice(0, 20).forEach(net => {
                        const option = new Option((net.encrypted?"🔒 ":"  ")+net.ssid+" ("+net.rssi+" dBm)", net.ssid);
